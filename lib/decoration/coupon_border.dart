...
///优惠券 外观 border
/// 可使用materia  和ShapeborderClipper
class CouponBorder extends ShapeBorder {
  final int holeCount;
  final double lineRate;
  final bool dash;
  final Color lineColor;

  CouponBorder({this.holeCount = 6, this.lineRate = 0.25, this.dash = true, this.lineColor = Colors.white});
  ...
  void paint(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    ...
    if (dash) {
      _drawDashLine(canvas, Offset(lineRate * rect.width, d), rect.height / 16, rect.height - 13, paint);
    } else {
      canvas.drawLine(Offset(lineRate * rect.width, d), Offset(lineRate * rect.width, rect.height - d), paint);
    }
  }
